...
export default function CompanyApplicantTable({ applicants, onShortlist, onReject, onApprove }) {
  ...
  return (
    <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-ambient overflow-hidden">
      <div className="grid grid-cols-12 gap-stack-md px-stack-lg py-stack-sm bg-surface-container-low font-label-sm text-label-sm uppercase tracking-wider text-on-surface-variant border-b border-outline-variant">
      ...
      {applicants.map((applicant) => (
        <div className="grid grid-cols-12 gap-stack-md px-stack-lg py-stack-md border-b border-outline-variant last:border-b-0 items-center" key={applicant.id}>
          <div className="col-span-4 flex items-center gap-stack-md min-w-0">
            {applicant.avatar ? (
          ...
          <div className="col-span-2 flex justify-center"><CompanyStatusBadge status={applicant.status} /></div>
          <div className="col-span-1 text-center text-on-surface-variant text-sm">{applicant.yearsExperience || 0} yrs</div>
