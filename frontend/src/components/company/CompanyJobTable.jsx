      ...
      {jobs.map((job) => (
        <div className="grid grid-cols-1 gap-4 px-stack-md sm:px-stack-lg py-stack-md border-t border-outline-variant items-start lg:grid-cols-12 lg:items-center" key={job.id}>
          ...
          <div className="lg:col-span-2 lg:text-center">
            <span className="text-on-surface-variant lg:hidden">Applicants: </span><Link className="text-secondary font-semibold hover:underline" to={`/company/jobs/${job.id}/applicants`}>{job.applicationsCount}</Link>
          </div>
          <div className="lg:col-span-2 lg:text-center text-on-surface-variant"><span className="lg:hidden">Views: </span>{job.views}</div>
          <div className="lg:col-span-2 flex justify-start lg:justify-center gap-2 overflow-x-auto pb-1">
            ...
            <button className="p-2 rounded-lg hover:bg-surface-container-high transition-colors text-on-surface-variant" title={job.status === 'active' ? 'Pause' : 'Publish'} onClick={() => onToggleStatus(job.id)} type="button">
              <span className="material-symbols-outlined text-[20px]">{job.status === 'active' ? 'pause' : 'publish'}</span>
            </button>
            <button className="p-2 rounded-lg text-error hover:bg-error-container transition-colors" title="Delete" onClick={() => onDeleteRequest(job)} type="button">
